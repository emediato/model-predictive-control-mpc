...
Ny = N2-N1+1;
Nu = [10,10,10]; %% horizonte de controle - dimensão 1 x m 
...
plot(refs(:,nin+1:nit)','-.','LineWidth',tamlinha,'Color',cores(4,:))
% ylim([0 2])
...
hl1 = legend('u_1','u_2','u_3','Location','NorthEast')
...
function [a] =applyPWM(in, out, i_init, i_sim, Ts_pwm, Ts_control, t_control, fsw, Vdc, Nu, N1, N2, delta, lambda)
...
%   fprintf('Vd: %.1f a %.1f V\n', min(in(1,ind)), max(in(1,ind)));
%   fprintf('Vq: %.1f a %.1f V\n', min(in(2,ind)), max(in(2,ind)));
