...
L_a = (v_t - R_a*i_a )/ 70 * v_t * 2*pi/60 * i_f
...
kpwm = 1/V_trip;        % Ganho PWM

%% MALHA DE CORRENTE - Projeto do Controlador PI
...
w_z = w_d/tan(M_p - pi/2 - phase_wd* pi/180); % Fase necessária do compensador

tau_z = 1/w_z;

k_ci = w_d/(sqrt(w_z^2 + w_d^2)*(mag_wd))
...
%% PLOTS USANDO FUNÇÃO BODE DO MATLAB
...
%% RESPOSTA AO DEGRAU - MALHA FECHADA DE CORRENTE
...
% G_cl_current = feedback(FTLA_comp, 1);
% 
% %% MALHA DE TENSÃO - Projeto do Controlador PI
...
% G_v = tf(K_v, [tau_z, 1]);  % Planta de tensão
% 
% fprintf('Função de transferência da planta de tensão G_v(s):\n');
% disp(G_v);
% 
% % FTLA inicial para tensão (considerando malha de corrente fechada)
...
% disp(FTLA_v_int);
% 
% % Avaliar FTLA_v_int na frequência w_c1
% [mag_wc1, phase_wc1] = bode(FTLA_v_int, w_d);
...
% 
% %Cálculo do compensador PI para tensão
...
% 
% %  Controlador PI para tensão: C_v(s) = k_o * (s + w_p) / s
% C_v = k_o * tf([1, w_p], [1, 0]);
% fprintf('\nControlador PI para tensão C_v(s):\n');
% disp(C_v);
% 
% % FTLA com compensador de tensão
...
% disp(G_cl_voltage);
% 
% %%  PLOTS DOS DIAGRAMAS DE BODE - MALHA DE TENSÃO
