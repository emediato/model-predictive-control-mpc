...
T = 5; Tsim=T;           % Tempo total de simulação

t_control = 0:Ts_control:(Tsim-Ts_control);
...
N1 = [4 1]; % inicio dos horizontes de predição - incluir atraso - dimensão 1 x n
N2 = [15 20]; % fim dos horizontes de predição - dimensão 1 x n
N = N2-N1+1; %horizonte de predição - não editar
Nq = [0]; % horizonte de predição da perturbação - dimensão 1 x mq

delta = [0.01 0.09]./N; %ponderação nos erros - dimensão 1 x n
lambda = [0.01 0.03]./Nu; %ponderação nas ações de controle - dimensão 1 x m       
...
%% definição das referências trifásicas
...
title(sprintf(['Horizontes Np = [%d]; Nc = [%d]; Nu=[%d %d], N1=[%d %d], N2=[%d %d], ', ...
               'delta=[%.4f %.4f], lambda=[%.4f %.4f]'], ...
               Np(1), Nc(1), Nu(1), Nu(2), N1(1), N1(2), N2(1), N2(2), ...
...
Va_ref = zeros(length(ind));
Vb_ref = zeros(length(ind));
Vc_ref = zeros(length(ind));
...
Van = (ga - 0.5) * Vdc;  % Tensão fase A-neutro
Vbn = (gb - 0.5) * Vdc;
Vcn = (gc - 0.5) * Vdc;
...
I_a_filtered = filter(b, a, Van) / R_load;
I_b_filtered = filter(b, a, Vbn) / R_load;
I_c_filtered = filter(b, a, Vcn) / R_load;

for k = 2:idxs
    % Equação: I[k+1] = I[k] + (Ts/L)*(V[k] - R*I[k])
    Ia(k) = Ia(k-1) + (Ts_pwm/L) * (Van(k-1) - R_load*Ia(k-1));
    Ib(k) = Ib(k-1) + (Ts_pwm/L) * (Vbn(k-1) - R_load*Ib(k-1));
    Ic(k) = Ic(k-1) + (Ts_pwm/L) * (Vcn(k-1) - R_load*Ic(k-1));
end
...
Id_meas = zeros(1,length(ind));
Iq_meas = zeros(1,length(ind));

Id_measured = zeros(1,length(ind));
Iq_measured = zeros(1,length(ind));
...
for k_ctrl = 1:ind
    ...
    Idq = K_park(theta_k) * [Ia(idx_nearest); 
                              Ib(idx_nearest); 
                              Ic(idx_nearest)];
...
title('Sinais PWM Gerados');

subplot(n_plots,1,4);
plot(t*1e3, Id_meas(1,:), 'r-', 'LineWidth', 2); hold on;
plot(t*1e3, Iq_meas(1,:), 'b-', 'LineWidth', 2);

subplot(n_plots,1,5);
plot(t, Id_measured, 'r-', 'LineWidth', 2); hold on;
plot(t, Iq_measured, 'b-', 'LineWidth', 2);
